...
def bam2bigwig(bam, genome, output, strand=None, scaled=True, verbose=1):
    ...
    #Sam edit - sort the genome coverage output with LC_COLLATE=C to get the bedGraphToBigWig to work
    sort_cmds = ['LC_COLLATE=C sort -k1,1 -k2,2n', x.fn,' > tempsort']
    if verbose:
	sys.stderr.write("[%s] Sorting bedGraph with LC_COLLATE=C: %s\n"%(datetime.ctime(datetime.now()), " ".join(sort_cmds)))
    os.system(" ".join(sort_cmds))
    
    
    # convert to bigWig
    cmds = ['bedGraphToBigWig', 'tempsort', faidx, output]
    if verbose:
